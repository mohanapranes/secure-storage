...
import com.thors.secure_store.metadata.FileMetadata;
...
public class FileDownloadService {
    ...
    public AllFilesListResponse getAllFilesForTheUser(String ownerId) {
        try {
            ...
            List<String> fileIds = metadataService.getAllFileMetadataForAUser(ownerId);
    ...
    public ResponseEntity<InputStreamResource> downloadFile(String fileId, String userId) {
        ...
        try {
            // 1. Load metadata
            FileMetadata metadata = metadataService.getFileMetadataForId(fileId);
            if (metadata == null) {
            ...
            // 4. Decrypt
            byte[] decryptedBytes =
                    encryptionService.decrypt(encryptedBytes, metadata.getEncryptedKey(), metadata.getIv());
